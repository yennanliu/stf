...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, minicap, display, screenOptions) {
    ...
    FrameProducer.prototype._startService = function() {
      log.info('Launching screen service %s', options.screenGrabber)
      if(options.screenGrabber === 'minicap-apk'){
        log.debug('starting minicap from apk %s', minicap.apk)
        return adb.shell(options.serial, util.format(
          'CLASSPATH=%s app_process /system/bin io.devicefarmer.minicap.Main',
          minicap.apk
        ))
      } else {
